...
function getValidationErrors()
{
    ...
    foreach ($fields as $field) {
        ...
        // Sanitize value
        if (isset($fdata[$fieldName]) && !is_array($fdata[$fieldName])) {
            $fdata[$fieldName] = htmlspecialchars($fdata[$fieldName]);
        }
...
/* Get the field name of the field to use as "Subject" in the email sent to the site owner */
function getCustomSubjectFieldName()
{
    global $fields;

    foreach ($fields as $field) {
        if (isset($field['use_as_subject']) && $field['use_as_subject'] == true) {
            return $field['name'];
        }
    }
...
/* Get the field name of the user field to use as "From Name" for sending the informations */
function getCustomEmailFromNameFieldName()
{
    global $fields;

    foreach ($fields as $field) {
        if (isset($field['from_name_this']) && $field['from_name_this'] == true) {
            return $field['name'];
        }
    }
...
{
    global $configs, $fdata, $lang, $fields;
    ...
    $customEmailFromNameFieldName = getCustomEmailFromNameFieldName();
    if ($customEmailFromFieldName != null) {
        $mail->From = $fdata[$customEmailFromFieldName];
        $mail->FromName = $customEmailFromNameFieldName ? $fdata[$customEmailFromNameFieldName] : '';
    } else {
        $mail->From = $configs['from_address'];
        $mail->FromName = $customEmailFromNameFieldName ? $fdata[$customEmailFromNameFieldName] : $configs['from_name'];
    }
    ...
    // Set subject
    $customSubjectFieldName = getCustomSubjectFieldName();
    if ($customSubjectFieldName != null) {
        $mail->Subject = $lang->emails_info->custom_before_subject.$fdata[$customSubjectFieldName];
    } else {
    ...
    foreach ($fields as $field) {
        ...
        switch ($field['type']) {
            case 'textarea':
                ...
                break;
            case 'select':
                if(is_array($fdata[$fieldName])) {
                    $value = '';
                    foreach ($fdata[$fieldName] as $v) {
                        if ($value)
                            $value .= ', ';
                        $value .= $v;
                    }
